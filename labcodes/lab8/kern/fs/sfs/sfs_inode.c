		...
		off_t offset, size_t *alenp, bool write)
{
	...
	*alenp = 0;
	 ...
	 *       NOTICE: useful function: sfs_bmap_load_nolock, sfs_buf_op	
	 */
	if ((blkoff = offset % SFS_BLKSIZE) != 0)//If offset isn't aligned with the first block
	{
	...
	size = SFS_BLKSIZE;
	while (nblks != 0)//Rd/Wr aligned blocks
	{
		if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0)
		{
			goto out;
	...
	if ((size = endpos % SFS_BLKSIZE) != 0)//If end position isn't aligned with the last block,
	{
		if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0)
		{
			goto out;
...
static int sfs_namefile(struct inode *node, struct iobuf *iob)
{
	struct sfs_disk_entry *entry;
	if (iob->io_resid
			<= 2|| (entry = kmalloc(sizeof(struct sfs_disk_entry))) == NULL)
	{
		return -E_NO_MEM;
	}
...
{ .vop_magic = VOP_MAGIC, .vop_open = sfs_opendir, .vop_close = sfs_close,
		.vop_fstat = sfs_fstat, .vop_fsync = sfs_fsync, .vop_namefile =
				sfs_namefile, .vop_getdirentry = sfs_getdirentry, .vop_reclaim =
				sfs_reclaim, .vop_gettype = sfs_gettype, .vop_lookup =
				sfs_lookup, };
/// The sfs specific FILE operations correspond to the abstract operations on a inode.
static const struct inode_ops sfs_node_fileops =
{ .vop_magic = VOP_MAGIC, .vop_open = sfs_openfile, .vop_close = sfs_close,
		.vop_read = sfs_read, .vop_write = sfs_write, .vop_fstat = sfs_fstat,
		.vop_fsync = sfs_fsync, .vop_reclaim = sfs_reclaim, .vop_gettype =
				sfs_gettype, .vop_tryseek = sfs_tryseek, .vop_truncate =
				sfs_truncfile, };
