...
{
SEG_NULL
, [SEG_KTEXT] = SEG(STA_X | STA_R, 0x0, 0xFFFFFFFF, DPL_KERNEL), [SEG_KDATA] = SEG(
		STA_W, 0x0, 0xFFFFFFFF, DPL_KERNEL), [SEG_UTEXT] = SEG(STA_X | STA_R, 0x0,
		0xFFFFFFFF, DPL_USER), [SEG_UDATA] = SEG(STA_W, 0x0, 0xFFFFFFFF, DPL_USER),
		[SEG_TSS] = SEG_NULL
, };
...
get_pte(pde_t *pgdir, uintptr_t la, bool create)
{
	...
	// (2) if entry is not present
	if (!(*pdep & PTE_P))
	{
		struct Page *page;
		// (3) if creating is needed, then alloc page for page table
	...
	pte_t pteIndex = PTX(la);

	//PDE_ADDR : address in page directory entry
	pde_t pdeAddr = PDE_ADDR(*pdep);

	//KADDR: takes a physical address and returns the corresponding kernel virtual address.
	pte_t *kAddr = KADDR(pdeAddr);

	return &kAddr[pteIndex];
}
...
#endif
	//(1) if page directory is present
	if (*ptep & PTE_P)
	{
